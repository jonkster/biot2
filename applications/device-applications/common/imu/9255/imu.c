...
{
    data->ts = xtimer_now64().ticks64 - t0;
    if (mpu9255_read_accel(&dev, &data->accel))
    {
        return false;
    }
    if (mpu9255_read_gyro(&dev, &data->gyro))
    {
        return false;
    }
    if (mpu9255_read_compass(&dev, &data->mag))
    {
        return false;
    }
    ...
    imuCalibrate(data);
    int32_t rawTemp;
    mpu9255_read_temperature(&dev, &rawTemp);
...
myQuat_t getPosition(mpu9255_t dev)
{
    ...
    if (getIMUData(dev, &imuData))
    {
        ...
        // NB MPU9255 has different XYZ axis for magnetometers than for gyros
...
bool initialiseIMU(mpu9255_t *dev)
{
    ...
    printf("+------------Initializing------------+\n");
    result = mpu9255_init(dev, I2C_0, MPU9255_HW_ADDR_HEX_68, MPU9255_COMP_ADDR_HEX_0C);
    ...
    result = mpu9255_set_gyro_fsr(dev, GFSR);
    if (result == -1) {
        puts("[Error] The given i2c is not enabled");
        return false;
    }
    ...
    result = mpu9255_set_accel_fsr(dev, AFSR);
    if (result == -1) {
        puts("[Error] The given i2c is not enabled");
        return false;
    }
    ...
    mpu9255_set_sample_rate(dev, GA_SAMPLE_RATE_HZ);
    ...
    mpu9255_set_compass_sample_rate(dev, C_SAMPLE_RATE_HZ);
