...
static gpio_t spi_cs = SPI_CS_UNDEF;
...
static int oledInterfaceInit(void)
{
    ...
    res =  spi_init_cs(spi_dev, spi_cs);
    if (res != SPI_OK)
    {
    ...
    res = spi_acquire(spi_dev, spi_cs, spi_mode, spi_speed);
    if (res == SPI_NOMODE)
    {
...
int oledWriteCommand(uint8_t command)
{
    ...
    mutex_lock(&mutex);
    oledSelectChip();
    if (spi_acquire(spi_dev, spi_cs, spi_mode, spi_speed) != SPI_OK)
    {
        puts("error: unable to acquire the SPI bus");
        return 1;
    }

    int res = spi_transfer_byte(spi_dev, spi_cs, true, command);
...
int oledWriteData(uint8_t data)
{
    ...
    int res = spi_transfer_byte(spi_dev, spi_cs, true, data);
